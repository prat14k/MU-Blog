...
session_start();
	...
	$r=mysql_query("select * from posts where postauthorid=$_SESSION[userid] order by  postdislikes desc limit 5") or die(mysql_error());
	//echo $r;
	//die();
	while($row=mysql_fetch_array($r))
	{
		$postname[$i]=new topposts();
		$postname[$i]->setname($row['postname']);
		$postname[$i]->setpoints($row['postdislikes']);
		$i++;
	}
	//print_r($postname);
